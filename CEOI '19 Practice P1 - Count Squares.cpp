...
int main(){
	...
	int h, v; vi x, y, dx, dy; int cnt = 0; cin >> h >> v;
	for(int i = 0; i < h; i++){int t; cin >> t; x.pb(t);}
    for(int i = 0; i < v; i++){int t; cin >> t; y.pb(t);}

    for(int i = 0; i < h-1; i++){
        for(int j = i+1; j < h; j++){dx.pb(x[j]-x[i]);}
    }
    for(int i = 0; i < v-1; i++){
        for(int j = i+1; j < v; j++){dy.pb(y[j]-y[i]);}
    }
