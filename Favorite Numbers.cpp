...
int main(){
	...
	int n, q; vi vec; cin >> n;
	for(int i = 0; i < n; i++){
        int t; cin >> t; vec.pb(t);
	}
	sort(vec.begin(), vec.end()); cin >> q; int hi = n-1; int lo = 0;
	...
	for(int i = 0; i < q; i++){
        ...
        while(true){
            ...
            if(vec[mi] <= t && vec[mi+1] > t){
                int j = mi-1; int cnt = 1;
                while(j >= 0){
                    j--; cnt++; if(vec[j] != t) break;
                }
                cout << vec[mi] << cnt << ell; break;
            }
        }
	}
