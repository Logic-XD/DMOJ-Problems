...
int main(){
    ...
    int n, x; cin >> n >> x; vector<pi> p; int ans;
    for(int i = 0; i < n; i++){
        int a, b; cin >> a >> b; p.pb({a, b});
    }
    for(int i = 0; i < (n-1); i++){
        if((p[i].f < p[i+1].f && p[i+1].s == 1 && p[i].s == 0) || (p[i].f > p[i+1].f && p[i].s == 1 && p[i+1].s == 0)) swap(p[i].f, p[i+1].f);
    }
    for(int i = 0; i < n; i++){
        if((p[i].f > x && p[i].s == 0) || (p[i].f <= x && p[i].s == 1)) ans++;
    }
    cout << ans;
