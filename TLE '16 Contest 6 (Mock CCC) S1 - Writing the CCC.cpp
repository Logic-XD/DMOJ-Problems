...
int main(){
	...
	int n, t; map<string, int> mp; vector<pi> vec; cin >> t;
    for(int i = 0; i < t; i++){string s; cin >> s; mp.insert({s, i});}

    cin >> n; for(int i = 0; i < n; i++){string s; cin >> s; vec.pb({mp[s], i+1});}

    sort(vec.begin(), vec.end());

    for(int i = 0; i < n; i++) cout << vec[i].s << '\n';
