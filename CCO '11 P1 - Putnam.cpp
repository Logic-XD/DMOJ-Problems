...
int main(){
    ...
    //inputs
    int n; vector<pair<double, ll>> v; cin >> n;
    for(int i = 0; i < n; i++){
        double rk; ll scr; cin >> scr >> rk; pair<double, ll> p = {rk, scr}; v.push_back(p);
    }
    ll scr; cin >> scr; sort(v.begin(), v.end());

    // find each interval
    double startint = 1;
    for(int i = 0; i < n; i++){
        double endint = 2*v[i].first - startint;
        if(v[i].second == scr){
            cout << (int)startint << '\n' << (int)endint << '\n';
        }
        startint = endint + 1;
