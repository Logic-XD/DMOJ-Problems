...
int main(){
	...
	int n; string s; cin >> n >> s; int m = s.size(); string s1, s2;
	for(int i = 0; i < n; i++){
        if(s[i] == '*') {s1 = s.substr(0, i); s2 = s.substr(i+1, m-1); break;}
	}

    for(int _ = 0; _ < n; _++){
        cin >> s; string ans = "DA"; if(s.size() < (m-1)) ans = "NE";
        for(int i = 0; i < s1.size(); i++){
            if(s1[i] != s[i]){ans = "NE"; break;}
        }
        for(int i = 0; i < s2.size(); i++){
            if(s2[s2.size()-i-1] != s[s.size()-i-1]){ans = "NE"; break;}
        }
        cout << ans << '\n';
    }
