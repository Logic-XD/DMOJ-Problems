...
int main() {
    int n, p; cin >> n >> p; vector<string> names(n); vi ppt(n);
    for(int i = 0; i < n; i++){
            cin >> names[i]; ppt[i] = 0;
    }
    for(int i = 0; i < p; i++){
        for(int j = 0; j < n; j++){
            int temp; cin >> temp; ppt[j] += temp;
        }
    }
    vector<pair<int, string>> v;
    for(int i = 0; i < n; i++){
        pair<int, string> p = {ppt[i], names[i]}; v.push_back(p);
    }
    sort(v.begin(), v.end(), greater<pair<int, string>>());
    for(int i = 0; i < n; i++){
        string rk = to_string(i+3)+'.'+' '; string name = v[i].second;
        cout << rk << name << endl;
    }
    return 0;
}
